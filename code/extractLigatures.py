...
for nPage in range(83, 96):
    ...
    #test commentaire
    colSources= ['Louvre 3226', 'Berlin 3029', 'Gurob', 'Rollin', 'Inena', 'Pentaour', 'Harris Th.', 'Harris H.M.', 'Abbott', 'nDm.t']
    resultsFolder=r'E:\personal\EtudesEgyptologie\séminaireEgypto\MollerHieratique\pdfextract'
    for w in wordsOnPage:
        ...
        if r>=0 and c>=0 and c<2:
            ...
            if not(key in rows[r]):
                ...
                rows[r][key]+=' '+w[4]
    #maintenant, on traite les images
    ims=page.get_images(full=True)
    for im in ims:
        imId=im[7]
        bb = page.get_image_bbox(imId)
        r,c = deduceRowAndColForItem(bb,rows,columns)
        if r>=0 and c>=0:
            if c==2:    #hierog
                rows[r]['hierog']=imId
            elif c>2:
                ...
                if not(key in rows[r]):
                    rows[r][key]=[]
                rows[r][key].append(imId)
    for row in rows:
        for cs in colSources:
            if cs in row:
                for imageId in row[cs]:
                    xref=list(filter(lambda item: item[7]==imageId, ims))[0][0]
                    pix=fitz.Pixmap(pdf_file, xref)
                    b=pix.tobytes("png")
                    fout=open(resultsFolder+"\\img\\"+imageId+"_hierat.png","wb")
                    fout.write(b)
                    fout.close()
                    #maintenant, recherche le hiéroglyphe correspondant
                    if 'hierog' in row:
                        hierogImageId=row['hierog']
                        xref=list(filter(lambda item: item[7]==hierogImageId, ims))[0][0]
                        pix=fitz.Pixmap(pdf_file, xref)
                        b=pix.tobytes("png")
                        fout=open(resultsFolder+"\\img\\"+imageId+"_hierog.png","wb")
                        fout.write(b)
                        fout.close()
                    else:
                        print("Missing for " + imageId + " - " + row['mdc'] + ' - ' + row['nmoller'])
                    sDescription="Manuel de Codage (MdC)\t"+row['mdc']+"\r"+"Text\t"+cs+"\r"
                    if "nmoller" in row:
                        sDescription+="N Möller\t"+row['nmoller']+'\r'
                    textfile=open(resultsFolder+'\\txt\\'+imageId+'.txt','wb')
                    textfile.write(sDescription.encode("utf8"))
                    textfile.close()